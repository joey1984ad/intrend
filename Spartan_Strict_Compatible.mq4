...
extern int MinBarsBetweenSignals = 3; // Minimum bars between same signal type to prevent clutter
extern bool StrictSignalFilter = true; // Only show one signal per trend direction until trend changes
...
int lastBuyBar = -1;
int lastSellBar = -1;
double lastTrendDirection = 0.0; // Track the last trend direction that generated a signal
                ...
                const int &spread[])
{
   ...
   for(int i = limit; i >= 0; i--)
   {
      ...
      if(up5 && up15 && up60)      trend[i] =  1.0;
                           ...
                           // draw arrow only on trend flips with signal filtering
        if(trend[i] != prev)
        {
           if(trend[i] > 0.0)
           {
              ...
              if(StrictSignalFilter && lastTrendDirection == 1.0)
              {
                 // If we already showed a buy signal for this trend, don't show another
                 canShowBuySignal = false;
              }
              else if(lastBuyBar >= 0)
              {
                 int barsSinceLastBuy = lastBuyBar - i;
                 if(barsSinceLastBuy < MinBarsBetweenSignals)
                    canShowBuySignal = false;
              }
              ...
              if(canShowBuySignal)
              {
                 ...
                 lastBuySignal = time[i];
                 lastBuyBar = i;
                 lastTrendDirection = 1.0;
              }
           }
           else
           {
              ...
              if(StrictSignalFilter && lastTrendDirection == -1.0)
              {
                 // If we already showed a sell signal for this trend, don't show another
                 canShowSellSignal = false;
              }
              else if(lastSellBar >= 0)
              {
                 int barsSinceLastSell = lastSellBar - i;
                 if(barsSinceLastSell < MinBarsBetweenSignals)
                    canShowSellSignal = false;
              }
